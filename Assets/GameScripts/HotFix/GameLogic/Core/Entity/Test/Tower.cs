...
namespace GameLogic
{
    public class Tower : IMemory
    {
        ...
        public ProjectileDataBase GetProjectileData(int level)
        {
            if (level < 0 || level > MaxLevel)
            {
                Log.Error("Param level '{0} invaild'", level);
        ...
        public static Tower Create(TowerDataBase towerData, int serialId, int level = 0)
        {
            Tower tower = PoolReference.Acquire<Tower>();
