...
namespace GameLogic
{
    ...
    public class CameraRigEditor : UnityEditor.Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            var fmh_42_69_638839874186840078 = Quaternion.LookRotation(Vector3.up); Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, size, snap,
                                                    Handles.RectangleHandleCap);
            var fmh_44_63_638839874186876716 = Quaternion.LookRotation(Vector3.up); Vector3 top = Handles.FreeMoveHandle(topPosition, size, snap,
                                                 Handles.RectangleHandleCap);
            var fmh_46_65_638839874186881694 = Quaternion.LookRotation(Vector3.up); Vector3 left = Handles.FreeMoveHandle(leftPosition, size, snap,
                                                  Handles.RectangleHandleCap);
            var fmh_48_67_638839874186886163 = Quaternion.LookRotation(Vector3.up); Vector3 right = Handles.FreeMoveHandle(rightPosition, size, snap,
                                                   Handles.RectangleHandleCap);
